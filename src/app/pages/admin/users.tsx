...
import { ActionButtons, DataTable, FormError, FormField, Modal, ModalActions, PageHeader, Pagination, rowBg, StatusBadge, StyledInput } from '../../components/shared';
...
export function Users() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (users.some((u) => u.email.toLowerCase() === formData.email.toLowerCase() && (!editingUser || u.id !== editingUser.id))) {
  ...
  if (loading) {
    return <Layout pageTitle="Users"><div className="flex items-center justify-center py-16"><Loader size={120} /></div></Layout>;
  }
    ...
    <Layout pageTitle="Users" onSearch={debouncedSearch} searchPlaceholder="Search users...">
      <div className="rounded-2xl border p-6" style={styles.card}>
        <PageHeader
          ...
          actions={
            <button onClick={() => setShowModal(true)} className="flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition-all hover:-translate-y-0.5" style={styles.btnPrimary}>
            ...
            <tr key={user.id} style={rowBg(i)}>
              <td className={`${styles.td} font-semibold`} style={{ color: 'var(--foreground)' }}>
                <div className="flex items-center gap-2.5">
                  <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full text-xs font-black text-white" style={{ background: 'linear-gradient(140deg, #0D1E5C, #3B6AEA)' }}>
              ...
              <td className={styles.td}>
                <ActionButtons onDelete={() => handleDelete(user.id)} extra={[{ icon: AlertTriangle, onClick: () => { setUserToDelete(user); setShowDeleteModal(true); }, title: 'Permanent Delete' }]} />
              </td>
            </tr>
          ))}
        />
      ...
      {showDeleteModal && (
        <Modal title="Permanent Delete" onClose={() => { setShowDeleteModal(false); setUserToDelete(null); }}>
          <div className="mb-4 -mt-2 flex items-center gap-3">
            <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl" style={{ background: 'rgba(239,68,68,0.12)' }}>
              <AlertTriangle className="h-5 w-5" style={{ color: 'var(--destructive)' }} />
            </div>
          </div>
          <div className="mb-5 rounded-xl border p-3" style={{ background: 'rgba(239,68,68,0.06)', borderColor: 'rgba(239,68,68,0.2)' }}>
            <p className="text-sm font-semibold" style={{ color: 'var(--foreground)' }}>{userToDelete?.first_name} {userToDelete?.last_name}</p>
            <p className="mt-0.5 text-xs" style={{ color: 'var(--muted-foreground)' }}>{userToDelete?.email}</p>
          </div>
          <p className="mb-5 text-xs font-semibold" style={{ color: 'var(--destructive)' }}>⚠️ This cannot be undone. All user data will be permanently removed.</p>
          <div className="flex gap-3">
            <button onClick={handlePermanentDelete} className="flex-1 rounded-xl py-2.5 text-sm font-bold text-white transition-all hover:-translate-y-0.5" style={styles.btnDanger}>Delete Permanently</button>
            <button onClick={() => { setShowDeleteModal(false); setUserToDelete(null); }} className="flex-1 rounded-xl border py-2.5 text-sm font-bold transition-colors" style={{ borderColor: 'var(--border)', color: 'var(--foreground)', background: 'var(--muted)' }}>Cancel</button>
          </div>
        </Modal>
      )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <FormField label={editingUser ? 'Password (leave empty to keep)' : 'Password'}>
              <div className="relative">
                <StyledInput type={showPassword ? 'text' : 'password'} value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} required={!editingUser} className="pr-10" />
            ...
            <FormField label="Contact No" error={formData.contact_no && formData.contact_no.length < 10 ? 'Must be 10 digits' : ''}>
              <StyledInput type="tel" value={formData.contact_no} onChange={(e) => { const v = e.target.value.replace(/\D/g, ''); if (v.length <= 10) setFormData({ ...formData, contact_no: v }); }} placeholder="1234567890" maxLength={10} required />
            </FormField>
