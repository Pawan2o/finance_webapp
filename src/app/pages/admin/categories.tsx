...
import { ActionButtons, DataTable, FormError, FormField, Modal, ModalActions, PageHeader, Pagination, rowBg, StyledInput, StyledSelect } from '../../components/shared';
...
export function Categories() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(d.non_field_errors?.[0] || d.name?.[0] || 'Failed to save category');
        return;
      }
      closeModal();
      fetchCategories(searchQuery, currentPage);
  ...
  if (loading) {
    return <Layout pageTitle="Categories"><div className="flex items-center justify-center py-16"><Loader size={120} /></div></Layout>;
  }
    ...
    <Layout pageTitle="Categories" onSearch={debouncedSearch} searchPlaceholder="Search categories...">
      <div className="rounded-2xl border p-6" style={styles.card}>
        <PageHeader
          ...
          actions={
            <button onClick={() => setShowModal(true)} className="flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition-all hover:-translate-y-0.5 sm:w-auto" style={styles.btnPrimary}>
        ...
        <div className="block space-y-2 sm:hidden">
          {categories.length > 0 ? categories.map((cat) => (
            <div key={cat.id} className="rounded-xl border p-4" style={{ borderColor: 'var(--border)', background: 'var(--muted)' }}>
              ...
              <p className="mt-1.5 text-xs font-medium" style={{ color: 'var(--muted-foreground)' }}>Type: {getTypeName(cat.type)}</p>
              <p className="mt-0.5 text-xs" style={{ color: 'var(--muted-foreground)' }}>{new Date(cat.created_at).toLocaleDateString()}</p>
            </div>
          )) : <p className="py-8 text-center text-sm" style={{ color: 'var(--muted-foreground)' }}>No categories found</p>}
        </div>
            ...
            <FormField label="Material Icon" hint="Enter a Material Icon name">
              <div className="flex items-center gap-2">
                ...
                {formData.material_icon && (
                  <span className="material-icons rounded-xl border p-2 text-xl" style={{ borderColor: 'var(--border)', background: 'var(--muted)', color: 'var(--foreground)' }}>
                    {formData.material_icon}
                  </span>
                )}
              </div>
